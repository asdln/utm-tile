...
bool make_tile(const std::string& shp_dir, const std::string& path, const std::string& dst_dir)
{
	...
	//16米以上分辨率图幅为100km，16米以下为20km
	double tile_projected_length = 100000.0;
	if (resolution < 16.0)
	{
		tile_projected_length = 20000.0;
	}
	...
	for (auto& shp_path : shp_paths)
	{
		...
		while ((poFeature = poLayer->GetNextFeature()) != NULL)
		{
			...
			if (env_dst.Intersects(env_mgrs))
			{
				...
				{
					std::string file_path_name = dst_dir + "/" + mgrs + ".tif";
					process_block(srs_dst, p.get(), resolution, tile_width_100km, tile_height_100km
